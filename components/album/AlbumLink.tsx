...
const AlbumLink: React.FC<Album> = ({ slug, image, name }) => {
  ...
  return (
    <Link
      ...
      key={slug}
      arial-label={slug}
      className="lg:w-[calc(50%-16px)] xl:w-[calc(50%-20px)] xl:h-[calc(50%-20px)] max-h-[50%] min-h-256 rounded-md overflow-hidden relative bg-black hover:bg-none"
    >
      <Image
        src={image.src}
        placeholder="blur"
        ...
        className="hover:blur-sm transition"
      />
